...
def decode(fileobj, elements_per_key):
	...
	for index, line in enumerate(fileobj):
		...
		if line.startswith("[") and not line.startswith("[%") and not line.startswith("[["):
			...
			translation = "" if line == "==" else line[3:]
...
def check_file(path):
	with open(path, encoding="utf-8") as fileobj:
		matches = re.findall(r"(BCLocalize|BCLocalizable)\s*\(\s*\"((?:(?:\\\")|[^\"])+)\"(?:\s*,\s*\"((?:(?:\\\")|[^\"])+)\")?\s*\)", fileobj.read())
	return matches
...
def languages():
	with open("../data/BestClient/languages/index.txt", encoding="utf-8") as f:
		index = decode(f, 3)
	langs = {"../data/BestClient/languages/" + key[0] + ".txt": [key[0]] + elements for key, elements in index.items()}
	return langs
