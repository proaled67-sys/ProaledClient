...
#include "components/proaledclient/r_jelly.h"
#include "components/proaledclient/r_trail.h"
...
namespace
{
	...
	{
		return CProaledClient::IsComponentDisabledByMask((int)CProaledClient::COMPONENT_GAMEPLAY_INPUT,
			g_Config.m_PcDisabledComponentsMaskLo, g_Config.m_PcDisabledComponentsMaskHi);
	}

	void RenderEyeComfortOverlay(CGameClient *pGameClient)
	{
		if(!g_Config.m_PcEyeComfort || pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_EYE_COMFORT))
			return;

		const float Strength = std::clamp(g_Config.m_PcEyeComfortStrength / 100.0f, 0.0f, 1.0f);
	...
	float EffectiveFastInputOffsetTicksFastMode()
	{
		if(!g_Config.m_TcFastInput ||
			PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) != 0 ||
			IsGameplayInputComponentDisabled())
			return 0.0f;

		// Mode 0: proaledclient fast input (ms based, 20ms per tick)
	...
	float EffectiveFastInputOffsetTicksBestMode(const CGameClient *pGameClient)
	{
		...
		if(!g_Config.m_TcFastInput ||
			PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) != 3 ||
			IsGameplayInputComponentDisabled())
			return 0.0f;
	...
	{
		if(!g_Config.m_TcFastInput ||
			PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) != 1 ||
			IsGameplayInputComponentDisabled())
			return 0.0f;
		if(g_Config.m_PcFastInputDeltaInput <= 0)
			return 0.0f;
		return g_Config.m_PcFastInputDeltaInput / 100.0f;
	}

	float EffectiveFastInputOffsetTicksSaikoPlusMode()
	{
		if(!g_Config.m_TcFastInput ||
			PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) != 4 ||
			IsGameplayInputComponentDisabled())
			return 0.0f;
		if(g_Config.m_PcSaikoPlusAmount <= 0)
			return 0.0f;
		return g_Config.m_PcSaikoPlusAmount / 100.0f;
	}

	float EffectiveFastInputOffsetTicks(const CGameClient *pGameClient)
	{
		const int FastInputMode = PcFastInputNormalizedMode(g_Config.m_PcFastInputMode);
		if(FastInputMode == 0)
	...
	int FastInputPredictionTicks(float OffsetTicks)
	{
		if(OffsetTicks <= 0.0f)
			return 0;
		if(PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 4)
	...
	float BestInputInterpolationAmount(float Fraction, float DeltaLength, bool Enable)
	{
		...
		switch(std::clamp(g_Config.m_PcBestInputInterpolation, 1, 3))
		{
		case 2:
			return CubicT;
		case 3:
	...
	{
		return PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 0 &&
	...
	{
		return PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 3 &&
		       g_Config.m_PcBestInputOthers != 0 &&
		       !IsGameplayInputComponentDisabled();
	}

	bool EffectiveDeltaInputOthers()
	{
		return PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 1 &&
		       g_Config.m_PcDeltaInputOthers != 0 &&
		       !IsGameplayInputComponentDisabled();
	}

	bool EffectiveSaikoPlusOthers()
	{
		return PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 4 &&
		       g_Config.m_PcSaikoPlusOthers != 0 &&
		       !IsGameplayInputComponentDisabled();
	}
...
{
	if(m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_OPTIMIZER))
		return false;
	return g_Config.m_PcOptimizer != 0;
}

bool CGameClient::OptimizerDisableParticles() const
{
	return OptimizerEnabled() && g_Config.m_PcOptimizerDisableParticles != 0;
}

bool CGameClient::OptimizerFpsFogEnabled() const
{
	return OptimizerEnabled() && g_Config.m_PcOptimizerFpsFog != 0;
}
...
void CGameClient::OptimizerFpsFogHalfExtents(float &HalfW, float &HalfH) const
{
	...
	{
		const float Radius = (float)g_Config.m_PcOptimizerFpsFogRadiusTiles * 32.0f;
		HalfW = Radius;
		HalfH = Radius;
		return;
	}
	...
	const float Percent = std::clamp(g_Config.m_PcOptimizerFpsFogZoomPercent, 1, 120) / 100.0f;
...
{
#if defined(CONF_FAMILY_WINDOWS)
	const bool WantDdnetHigh = OptimizerEnabled() && g_Config.m_PcOptimizerDdnetPriorityHigh != 0;
	...
	static const wchar_t *const s_apDiscordExeNames[] = {L"Discord.exe", L"DiscordPTB.exe", L"DiscordCanary.exe"};
	const bool WantDiscordBelow = OptimizerEnabled() && g_Config.m_PcOptimizerDiscordPriorityBelowNormal != 0;
...
void CGameClient::RenderOptimizerFpsFogRect()
{
	...
	if(!OptimizerFpsFogEnabled() || g_Config.m_PcOptimizerFpsFogRenderRect == 0)
		return;
...
void CGameClient::OnConsoleInit()
{
	...
	m_vpAll.insert(m_vpAll.end(), {&m_Skins,
					      ...
					      &m_Translate, // TClient
					      &m_Ghost,
					      &m_ProaledClient, // ProaledClient binds
...
void CGameClient::InitializeLanguage()
{
	...
	// ProaledClient
	str_format(aBuf, sizeof(aBuf), "ProaledClient/%s", g_Config.m_ClLanguagefile);
	g_Localization.Load(aBuf, Storage(), Console(), false);
}
...
void CGameClient::OnRender()
{
	...
	const bool UseGameNoHudAspect = IsActiveGameplay && g_Config.m_PcCustomAspectRatioApplyMode == 2;
...
{
	if(g_Config.m_PcBestInputPreset == 1)
		return {270, 55, 15};
	if(g_Config.m_PcBestInputPreset == 2)
		return {300, 35, 15};
	if(g_Config.m_PcBestInputPreset == 3)
	{
		if(!m_Snap.m_pLocalInfo)
			return {0, 0, 0};
	...
	return {
		std::clamp(g_Config.m_PcBestInputOffset, 0, 1000),
		std::clamp(g_Config.m_PcBestInputSmoothing, 0, 100),
		std::clamp(g_Config.m_PcBestInputLatencyComp, 0, 50),
	};
}
...
void CGameClient::UpdateAutoTeamLock()
{
	...
	if(!g_Config.m_PcAutoTeamLock || m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_GAMEPLAY_AUTO_TEAM_LOCK))
	{
	...
	{
		const int DelayTicks = g_Config.m_PcAutoTeamLockDelay * Client()->GameTickSpeed();
...
void CGameClient::ProcessEvents()
{
	...
	for(int Index = 0; Index < Num; Index++)
	{
		...
		else if(Item.m_Type == NETEVENTTYPE_SOUNDWORLD)
		{
			...
			if(g_Config.m_PcMuteOthersHook)
			{
...
void CGameClient::OnPredict()
{
	...
	for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= FinalTickSelf; Tick++)
	{
		...
		if(FastInputTicks > 0 && Tick > FinalTickRegular)
		{
			...
			if(g_Config.m_PcFastInputMode != 4 && GetDummyFastInput(DummyFastInput, pDummyInputData, pDummyChar, LocalTee, DummyTee))
		...
		RealPredTick && m_PredictedTick >= MIN_TICK)
	{
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(g_Config.m_PcFastInputMode == 0 && EffectiveFastInputOthers() && FastInputTicks > 0)
...
vec2 CGameClient::GetSmoothPos(int ClientId)
{
	...
	const bool BestInputInterpolationEnabled = g_Config.m_PcFastInputMode == 3 && FastInputTicksClient > 0 && (ClientId == m_Snap.m_LocalClientId || FastInputOthers);
...
vec2 CGameClient::GetFastInputPos(int ClientId)
{
	...
	const bool FastInputOthers = EffectiveAnyFastInputOthers();
	const int FastInputTicksClient = ClientId == m_Snap.m_LocalClientId ? FastInputTicks : (FastInputOthers ? FastInputPredictionTicksOthers(FastInputOffsetTicks) : 0);
	const bool BestInputInterpolationEnabled = g_Config.m_PcFastInputMode == 3 && FastInputTicks > 0;
...
vec2 CGameClient::GetFreezePos(int ClientId)
{
	...
	const bool BestInputInterpolationEnabled = g_Config.m_PcFastInputMode == 3 && (ApplyFastInputLocal || ApplyFastInputOthers);
