...
void CSpectator::OnRender()
{
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		const bool ShowProaledClientIndicator = g_Config.m_PcClientIndicatorInScoreboard &&
						     pInfo->m_ClientId >= 0 &&
						     GameClient()->m_ClientIndicator.IsPlayerProaledClient(pInfo->m_ClientId);
		const float ProaledClientIconSize = FontSize * (0.8f + 0.3f * g_Config.m_PcClientIndicatorInSoreboardSize / 100.0f);
		const float ProaledClientIconSpacing = 4.0f;
		const float ProaledClientIconReserve = ShowProaledClientIndicator ? ProaledClientIconSize + ProaledClientIconSpacing : 0.0f;
		const float NameX = Width / 2.0f + x + 50.0f + ProaledClientIconReserve;
		...
		NameCursor.m_LineWidth = 180.0f - ProaledClientIconReserve;
		if(NameCursor.m_LineWidth < 0.0f)
			NameCursor.m_LineWidth = 0.0f;

		if(ShowProaledClientIndicator)
		{
			const CUIRect IconRect = {
				NameX - ProaledClientIconReserve,
				Height / 2.0f + y + BoxMove + (LineHeight - ProaledClientIconSize) / 2.0f,
				ProaledClientIconSize,
				ProaledClientIconSize};
