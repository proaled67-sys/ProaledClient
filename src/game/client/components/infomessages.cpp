...
void CInfoMessages::OnRender()
{
	...
	for(int i = 1; i <= MAX_INFOMSGS; i++)
	{
		...
		if(InfoMsg.m_Type == EType::TYPE_KILL && g_Config.m_ClShowKillMessages)
		{
			...
			if(BCUiAnimations::Enabled() && g_Config.m_PcKillfeedAnimation != 0 && g_Config.m_PcKillfeedAnimationMs > 0 && InfoMsg.m_AppearTime > 0)
			{
				const float Duration = BCUiAnimations::MsToSeconds(g_Config.m_PcKillfeedAnimationMs);
