...
void CMod::OnFire(bool Pressed)
{
	...
	const CGameClient::CClientData *pProaledClient = nullptr;
	float ProaledClientScore = -INFINITY;
	if(GameClient()->m_Snap.m_SpecInfo.m_Active || Player.m_Team == TEAM_SPECTATORS)
	{
		...
		for(const CGameClient::CClientData &Other : GameClient()->m_aClients)
		{
			...
			if(Score > ProaledClientScore)
			{
				ProaledClientScore = Score;
				pProaledClient = &Other;
			}
		}
	}
	else
	...
	m_ModWeaponActiveId = pProaledClient->ClientId();
