...
static bool IsTClientTabDisabledByComponents(const CGameClient *pGameClient, int Tab)
{
	...
	case TCLIENT_TAB_SETTINGS:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_SETTINGS_TAB);
	case TCLIENT_TAB_BINDWHEEL:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_BIND_WHEEL_TAB);
	case TCLIENT_TAB_WARLIST:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_WAR_LIST_TAB);
	case TCLIENT_TAB_BINDCHAT:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_CHAT_BINDS_TAB);
	case TCLIENT_TAB_STATUSBAR:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_STATUS_BAR_TAB);
	case TCLIENT_TAB_INFO:
		return pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_INFO_TAB);
	default:
		return false;
	}
}

static bool IsTClientSettingsSectionDisabledByComponents(const CGameClient *pGameClient, CProaledClient::EProaledClientComponent Component)
{
	return pGameClient->m_ProaledClient.IsComponentDisabled(Component);
}
...
void CMenus::RenderSettingsTClientSettings(CUIRect MainView)
{
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_VISUAL))
	{
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_ANTI_LATENCY))
	{
		// ***** Anti Latency Tools ***** //
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_ANTI_PING_SMOOTHING))
	{
		// ***** Improved Anti Ping ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_AUTO_EXECUTE))
	{
		// ***** Execute on join ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_VOTING))
	{
		// ***** Voting ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_AUTO_REPLY))
	{
		// ***** Auto Reply ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_PLAYER_INDICATOR))
	{
		// ***** Player Indicator ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_PET))
	{
		// ***** Pet ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_HUD))
	{
		// ***** HUD ***** //
		Column.HSplitTop(Margin, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_FROZEN_TEE_DISPLAY))
	{
		// ***** Frozen Tee Display ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_TILE_OUTLINES))
	{
		// ***** Tile Outlines ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_GHOST_TOOLS))
	{
		// ***** Ghost Tools ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_RAINBOW))
	{
		// ***** Rainbow ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_TEE_TRAILS))
	{
		// ***** Tee Trails ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_BACKGROUND_DRAW))
	{
		// ***** BG Draw ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
	...
	if(!IsTClientSettingsSectionDisabledByComponents(GameClient(), CProaledClient::COMPONENT_TCLIENT_SETTINGS_FINISH_NAME))
	{
		// ***** Finish Name ***** //
		Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
		s_SectionBoxes.push_back(Column);
...
void CMenus::RenderSettingsTClientConfigs(CUIRect MainView)
{
	...
	ChangesCount = s_StagedInts.size() + s_StagedStrs.size() + s_StagedCols.size();
	{
		...
		if(DoButton_CheckBox(&g_Config.m_TcUiShowProaledClient, Localize("ProaledClient"), g_Config.m_TcUiShowProaledClient, &TopCol3))
			g_Config.m_TcUiShowProaledClient ^= 1;
	}
	...
	auto DomainEnabled = [&](ConfigDomain Domain) {
		...
		if(Domain == ConfigDomain::PROALEDCLIENT)
			return g_Config.m_TcUiShowProaledClient != 0;
		// only show DDNet, TClient and ProaledClient domains
		return false;
	};
	...
	auto DomainName = [](ConfigDomain D) {
		...
		case ConfigDomain::PROALEDCLIENT: return "ProaledClient";
