...
namespace HudLayout
{
	...
	namespace
	{
		...
		SModuleLayout ConfigLayout(EModule Module)
		{
			...
			case MODULE_MUSIC_PLAYER:
				return {(float)g_Config.m_PcHudMusicPlayerX, (float)g_Config.m_PcHudMusicPlayerY, g_Config.m_PcHudMusicPlayerScale, Runtime.m_Mode, Runtime.m_Enabled, Runtime.m_BackgroundEnabled, Runtime.m_BackgroundColor};
			case MODULE_VOICE_TALKERS:
				return {(float)g_Config.m_PcHudVoiceHudX, (float)g_Config.m_PcHudVoiceHudY, g_Config.m_PcHudVoiceHudScale, Runtime.m_Mode, Runtime.m_Enabled, Runtime.m_BackgroundEnabled, Runtime.m_BackgroundColor};
			case MODULE_VOICE_STATUS:
				return {(float)g_Config.m_PcHudVoiceMuteIconsX, (float)g_Config.m_PcHudVoiceMuteIconsY, g_Config.m_PcHudVoiceMuteIconsScale, Runtime.m_Mode, Runtime.m_Enabled, Runtime.m_BackgroundEnabled, Runtime.m_BackgroundColor};
			case MODULE_CHAT:
				return {(float)g_Config.m_PcHudChatX, (float)g_Config.m_PcHudChatY, g_Config.m_PcHudChatScale, Runtime.m_Mode, Runtime.m_Enabled, Runtime.m_BackgroundEnabled, Runtime.m_BackgroundColor};
			case MODULE_VOTES:
				return {(float)g_Config.m_PcHudVotesX, (float)g_Config.m_PcHudVotesY, g_Config.m_PcHudVotesScale, Runtime.m_Mode, Runtime.m_Enabled, Runtime.m_BackgroundEnabled, Runtime.m_BackgroundColor};
			default:
				return Runtime;
			}
		}
		...
		void WriteConfigLayout(EModule Module, const SModuleLayout &Layout)
		{
			...
			{
			case MODULE_MUSIC_PLAYER:
				g_Config.m_PcHudMusicPlayerX = round_to_int(Layout.m_X);
				g_Config.m_PcHudMusicPlayerY = round_to_int(Layout.m_Y);
				g_Config.m_PcHudMusicPlayerScale = Layout.m_Scale;
				break;
			case MODULE_VOICE_TALKERS:
				g_Config.m_PcHudVoiceHudX = round_to_int(Layout.m_X);
				g_Config.m_PcHudVoiceHudY = round_to_int(Layout.m_Y);
				g_Config.m_PcHudVoiceHudScale = Layout.m_Scale;
				break;
			case MODULE_VOICE_STATUS:
				g_Config.m_PcHudVoiceMuteIconsX = round_to_int(Layout.m_X);
				g_Config.m_PcHudVoiceMuteIconsY = round_to_int(Layout.m_Y);
				g_Config.m_PcHudVoiceMuteIconsScale = Layout.m_Scale;
				break;
			case MODULE_CHAT:
				g_Config.m_PcHudChatX = round_to_int(Layout.m_X);
				g_Config.m_PcHudChatY = round_to_int(Layout.m_Y);
				g_Config.m_PcHudChatScale = Layout.m_Scale;
				break;
			case MODULE_VOTES:
				g_Config.m_PcHudVotesX = round_to_int(Layout.m_X);
				g_Config.m_PcHudVotesY = round_to_int(Layout.m_Y);
				g_Config.m_PcHudVotesScale = Layout.m_Scale;
				break;
			default:
				break;
			}
	...
	SModuleLayout Get(EModule Module, float HudWidth, float HudHeight)
	{
		...
		{
			const float UserScale = std::clamp(g_Config.m_PcHookComboSize / 100.0f, 0.5f, 2.0f);
