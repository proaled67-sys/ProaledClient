...
void CMenus::RenderSettingsSound(CUIRect MainView)
{
	...
	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_PcMuteOthersHook, Localize("Mute hook sounds of other players"), g_Config.m_PcMuteOthersHook, &Button))
		g_Config.m_PcMuteOthersHook ^= 1;

	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_PcMuteOthersHammer, Localize("Mute hammer sounds of other players"), g_Config.m_PcMuteOthersHammer, &Button))
		g_Config.m_PcMuteOthersHammer ^= 1;
	...
	MainView.HSplitTop(20.0f, &Button, &MainView);
	if(DoButton_CheckBox(&g_Config.m_PcMenuSfx, Localize("Enable menu UI sounds (from osu!)"), g_Config.m_PcMenuSfx, &Button))
		g_Config.m_PcMenuSfx ^= 1;

	{
		MainView.HSplitTop(5.0f, nullptr, &MainView);
		MainView.HSplitTop(20.0f, &Button, &MainView);
		Ui()->DoScrollbarOption(&g_Config.m_PcMenuSfxVolume, &g_Config.m_PcMenuSfxVolume, &Button, Localize("SFX sound volume"), 0, 100, &CUi::ms_LogarithmicScrollbarScale, 0u, "%");
	}
...
{
	const bool ProaledClientPageVisible = g_Config.m_UiSettingsPage == SETTINGS_PROALEDCLIENT;
	if(!ProaledClientPageVisible && (m_AssetsEditorState.m_VisualsEditorOpen || m_AssetsEditorState.m_VisualsEditorInitialized))
		m_AssetsEditorState.m_VisualsEditorOpen = false;
	if(!ProaledClientPageVisible && m_ComponentsEditorState.m_Open)
		ComponentsEditorCloseNow();

	if(g_Config.m_UiSettingsPage == SETTINGS_PROALEDCLIENT && m_AssetsEditorState.m_VisualsEditorOpen && m_AssetsEditorState.m_FullscreenOpen)
	{
		RenderSettingsProaledClient(MainView);
		return;
	}
	if(g_Config.m_UiSettingsPage == SETTINGS_PROALEDCLIENT && m_ComponentsEditorState.m_Open && m_ComponentsEditorState.m_FullscreenOpen)
	{
		RenderSettingsProaledClient(MainView);
		return;
	}
	...
	auto RenderSettingsPage = [&](CUIRect PageView) {
		...
		else if(g_Config.m_UiSettingsPage == SETTINGS_PROALEDCLIENT)
		{
			GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_RESERVED0);
			RenderSettingsProaledClient(PageView);
		}
		else if(g_Config.m_UiSettingsPage == SETTINGS_PROFILES)
		{
			GameClient()->m_MenuBackground.ChangePosition(14);
			if(GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_PROFILES_PAGE))
		...
		else if(g_Config.m_UiSettingsPage == SETTINGS_CONFIGS)
		{
			...
			if(GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_TCLIENT_CONFIGS_PAGE))
				g_Config.m_UiSettingsPage = SETTINGS_TCLIENT;
			else
	...
	g_Config.m_PcSettingsLayout = minimum(maximum(g_Config.m_PcSettingsLayout, 0), 1);

	if(g_Config.m_PcSettingsLayout == 0)
	{
		...
		if(ActiveRootTab != ROOT_TAB_PROALEDCLIENT)
		{
			CUIRect SubTabBar;
...
void CMenus::RenderSettingsDDNet(CUIRect MainView)
{
...
#if defined(CONF_AUTOUPDATE)
	{
		...
		{
			str_format(aBuf, sizeof(aBuf), "ProaledClient %s Is release", GameClient()->m_ProaledClient.m_aVersionStr);
		...
		{
			str_copy(aBuf, Localize("ProaledClient Client updated!"));
