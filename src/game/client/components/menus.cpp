...
{
	ConfigManager()->RegisterCallback(CMenus::ConfigSaveCallback, this, ConfigDomain::PROALEDCLIENT);
...
void CMenus::OnInit()
{
	...
	m_MainMenuLogoTexture = Graphics()->LoadTexture("proaledclient/gui_logo.png", IStorage::TYPE_ALL);
...
{
	if(SampleId < 0 || !g_Config.m_SndEnable || !g_Config.m_PcMenuSfx)
		return;

	const float Volume = std::clamp(g_Config.m_PcMenuSfxVolume / 100.0f, 0.0f, 1.0f);
	if(Volume <= 0.0f)
		return;
...
void CMenus::Render()
{
	...
	if(BrowserPageActive && g_Config.m_PcAutoServerListRefresh)
	{
		const bool BrowserBusy = ServerBrowser()->IsRefreshing() || ServerBrowser()->IsGettingServerlist();
		if(!BrowserBusy)
		{
			const int64_t Now = time_get();
			const int64_t RefreshInterval = (int64_t)g_Config.m_PcAutoServerListRefreshSeconds * time_freq();
	...
	else
	{
		...
		const bool MenuMediaBackgroundDisabled = GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_MEDIA_BACKGROUND);
		m_MenuMediaBackground.SyncFromConfig(MenuMediaBackgroundDisabled ? 0 : g_Config.m_PcMenuMediaBackground, g_Config.m_PcMenuMediaBackgroundPath);
	...
	{
		const bool IngameMenuAnimationEnabled = BCUiAnimations::Enabled() && g_Config.m_PcIngameMenuAnimation != 0 && g_Config.m_PcIngameMenuAnimationMs > 0;
		const float Ease = IngameMenuAnimationEnabled ? BCUiAnimations::EaseInOutQuad(m_PcIngameMenuOpenPhase) : 1.0f;
	...
	case IClient::STATE_OFFLINE:
		...
		{
			const bool FullscreenProaledClientEditor =
				m_MenuPage == PAGE_SETTINGS &&
				g_Config.m_UiSettingsPage == SETTINGS_PROALEDCLIENT &&
			...
			if(FullscreenProaledClientEditor)
				MainView = Screen;
			else
...
void CMenus::SetActive(bool Active)
{
	...
	if(!m_MenuActive)
	{
		m_PcIngameMenuClosing = false;
		m_PcIngameMenuOpenPhase = 0.0f;
		if(m_NeedSendinfo)
		{
	...
	else
	{
		...
		if(Client()->State() == IClient::STATE_ONLINE || Client()->State() == IClient::STATE_DEMOPLAYBACK)
			m_PcIngameMenuOpenPhase = (BCUiAnimations::Enabled() && g_Config.m_PcIngameMenuAnimation != 0 && g_Config.m_PcIngameMenuAnimationMs > 0) ? 0.0f : 1.0f;
		else
			m_PcIngameMenuOpenPhase = 1.0f;
	}
}
...
bool CMenus::OnInput(const IInput::CEvent &Event)
{
	...
	if((Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == KEY_ESCAPE) || IsActive())
	{
		if((Event.m_Flags & IInput::FLAG_PRESS) && Event.m_Key == KEY_ESCAPE &&
			...
			BCUiAnimations::Enabled() && g_Config.m_PcIngameMenuAnimation != 0 && g_Config.m_PcIngameMenuAnimationMs > 0 &&
			!Ui()->IsPopupOpen())
		{
			if(!m_PcIngameMenuClosing)
			{
				PlayIngameMenuCloseSfx();
				m_MenuSfxSuppressNextIngameClose = true;
			}
			m_PcIngameMenuClosing = true;
			return true;
		}
...
void CMenus::OnRender()
{
	...
	const bool UseWindowAspectForUi = IngameMenu && g_Config.m_PcCustomAspectRatioApplyMode != 1;
	Ui()->SetUseGraphicsScreenAspect(!UseWindowAspectForUi);
	const bool IngameMenuAnimated = IngameMenu && BCUiAnimations::Enabled() && g_Config.m_PcIngameMenuAnimation != 0 && g_Config.m_PcIngameMenuAnimationMs > 0;
	if(IngameMenuAnimated)
	{
		const bool AllowInput = !ConsoleActive && !m_PcIngameMenuClosing && m_PcIngameMenuOpenPhase >= 0.999f;
	...
	if(IngameMenu)
	{
		if(BCUiAnimations::Enabled() && g_Config.m_PcIngameMenuAnimation != 0 && g_Config.m_PcIngameMenuAnimationMs > 0)
		{
			const float Target = m_PcIngameMenuClosing ? 0.0f : 1.0f;
			BCUiAnimations::UpdatePhase(m_PcIngameMenuOpenPhase, Target, Client()->RenderFrameTime(), BCUiAnimations::MsToSeconds(g_Config.m_PcIngameMenuAnimationMs));
			if(m_PcIngameMenuClosing && m_PcIngameMenuOpenPhase <= 0.0f)
			{
				Ui()->SetEnabled(true);
	...
	if(s_StuckLastMode != g_Config.m_PcCustomAspectRatioMode || s_StuckLastRatio != g_Config.m_PcCustomAspectRatio)
	{
		s_StuckOverlayDismissed = false;
		s_StuckLastMode = g_Config.m_PcCustomAspectRatioMode;
		s_StuckLastRatio = g_Config.m_PcCustomAspectRatio;
	}
	const bool AspectOverlayActive = g_Config.m_PcCustomAspectRatioApplyMode == 1 &&
		(g_Config.m_PcCustomAspectRatioMode > 0 ||
		(g_Config.m_PcCustomAspectRatioMode < 0 && g_Config.m_PcCustomAspectRatio > 0));
...
void CMenus::QuitWithMenuSfx()
{
	...
	const bool CanDelayQuitForSfx = g_Config.m_SndEnable && g_Config.m_PcMenuSfx && ExitSample >= 0;
