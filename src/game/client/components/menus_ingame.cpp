...
void CMenus::RenderGame(CUIRect MainView)
{
	...
	const bool ShowEscPlayersCarousel = g_Config.m_PcEscPlayerList != 0 && (!g_Config.m_ClTouchControls || !GameClient()->m_TouchControls.IsEditingActive());
...
void CMenus::RenderEscPlayersCarousel(CUIRect MainView)
{
	...
	for(int i = 0; i < NumPlayers; i++)
	{
		...
		GameClient()->m_ProaledClient.SanitizePlayerName(GameClient()->m_aClients[ClientId].m_aName, aSanitizedName, sizeof(aSanitizedName), ClientId, true);
...
void CMenus::RenderPlayers(CUIRect MainView)
{
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		GameClient()->m_ProaledClient.SanitizePlayerName(CurrentClient.m_aName, aSanitizedName, sizeof(aSanitizedName), Index);
		GameClient()->m_ProaledClient.SanitizeText(CurrentClient.m_aClan, aSanitizedClan, sizeof(aSanitizedClan));
		...
		Button.VSplitLeft((Width - Button.h) / 4.0f, nullptr, &Button);
		Button.VSplitLeft(Button.h, &Button, nullptr);
		if(GameClient()->m_ProaledClient.HasStreamerFlag(CProaledClient::STREAMER_HIDE_FRIEND_WHISPER))
...
void CMenus::RenderServerInfo(CUIRect MainView)
{
	...
	GameClient()->m_ProaledClient.SanitizeText(CurrentServerInfo.m_aName, aSanitizedServerName, sizeof(aSanitizedServerName));
	Ui()->DoLabel(&Label, aSanitizedServerName, FontSizeBody, TEXTALIGN_ML);

	ServerInfo.HSplitTop(FontSizeBody, &Label, &ServerInfo);
	char aBuf[256];
	char aMaskedAddress[sizeof(CurrentServerInfo.m_aAddress)];
	GameClient()->m_ProaledClient.MaskServerAddress(CurrentServerInfo.m_aAddress, aMaskedAddress, sizeof(aMaskedAddress));
	...
	// copy info button
	{
		...
		if(DoButton_Menu(&s_CopyButton, Localize("Copy info"), 0, &Button))
		{
			...
			GameClient()->m_ProaledClient.SanitizePlayerName(Client()->PlayerName(), aSanitizedOwnName, sizeof(aSanitizedOwnName), GameClient()->m_aLocalIds[g_Config.m_ClDummy]);
			str_format(
				aInfo,
				sizeof(aInfo),
				"%s\n"
