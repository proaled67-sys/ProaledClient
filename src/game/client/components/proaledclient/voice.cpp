...
{
	return g_Config.m_PcVoiceDebug != 0;
}
...
static constexpr const char *VOICE_FORCED_SERVER_ADDR = "server.proaled-client.ru:9987";
...
{
	const float Gain = std::clamp(Config.m_PcVoiceMicVolume / 100.0f, 0.0f, 3.0f);
	if(Gain == 1.0f)
		return;
	...
	for(int i = 0; i < Count; i++)
	{
		...
		if(Config.m_PcVoiceFilterEnable)
...
static void ApplyHpfCompressor(const SRClientVoiceConfigSnapshot &Config, int16_t *pSamples, int Count, float &PrevIn, float &PrevOut, float &Env, float &GainState)
{
	...
	const float Threshold = std::clamp(Config.m_PcVoiceCompThreshold / 100.0f, 0.01f, 1.0f);
	const float Ratio = std::max(1.0f, Config.m_PcVoiceCompRatio / 10.0f);
	const float AttackSec = std::max(0.001f, Config.m_PcVoiceCompAttackMs / 1000.0f);
	const float ReleaseSec = std::max(0.001f, Config.m_PcVoiceCompReleaseMs / 1000.0f);
	const float MakeupGain = std::clamp(Config.m_PcVoiceCompMakeup / 100.0f, 0.0f, 2.0f);
	const float Limiter = std::clamp(Config.m_PcVoiceLimiter / 100.0f, 0.05f, 1.0f);
...
{
	if(!Config.m_PcVoiceNoiseSuppressEnable)
		return;

	const float Strength = std::clamp(Config.m_PcVoiceNoiseSuppressStrength / 100.0f, 0.0f, 1.0f);
	if(Strength <= 0.0f)
		return;
...
void CProaledClientVoice::SDLAudioCallback(void *pUserData, Uint8 *pStream, int Len)
{
	auto *pThis = static_cast<CProaledClientVoice *>(pUserData);
	if(!pThis || Len <= 0)
		return;
...
void CProaledClientVoice::Init(CGameClient *pGameClient, IClient *pClient, IConsole *pConsole)
{
...
void CProaledClientVoice::OnShutdown()
{
	Shutdown();
}

void CProaledClientVoice::SetPttActive(bool Active)
{
	...
	if(WasActive)
	{
		const int DelayMs = std::clamp(g_Config.m_PcVoicePttReleaseDelayMs, 0, 1000);
		if(DelayMs > 0)
...
bool CProaledClientVoice::EnsureSocket()
{
	if(m_Socket)
		return true;
...
bool CProaledClientVoice::EnsureAudio()
{
	...
	const bool WantStereo = g_Config.m_PcVoiceStereo != 0;
	...
	const bool BackendChanged = str_comp(m_aAudioBackend, g_Config.m_PcVoiceAudioBackend) != 0;
	if(BackendChanged)
	{
		if(m_CaptureDevice)
		{
		...
		m_MixBuffer.clear();
		str_copy(m_aAudioBackend, g_Config.m_PcVoiceAudioBackend, sizeof(m_aAudioBackend));
	...
	const char *pRequestedBackend = g_Config.m_PcVoiceAudioBackend[0] ? g_Config.m_PcVoiceAudioBackend : nullptr;
	...
	if(str_comp(m_aInputDeviceName, g_Config.m_PcVoiceInputDevice) != 0)
	{
		if(m_CaptureDevice)
		{
			SDL_CloseAudioDevice(m_CaptureDevice);
		...
		str_copy(m_aInputDeviceName, g_Config.m_PcVoiceInputDevice, sizeof(m_aInputDeviceName));
		m_LogDeviceChange = true;
		m_CaptureUnavailable = false;
		m_LastAudioRetryAttempt = 0;
	}

	if(str_comp(m_aOutputDeviceName, g_Config.m_PcVoiceOutputDevice) != 0)
	{
		if(m_OutputDevice)
		{
			SDL_CloseAudioDevice(m_OutputDevice);
		...
		str_copy(m_aOutputDeviceName, g_Config.m_PcVoiceOutputDevice, sizeof(m_aOutputDeviceName));
		m_LogDeviceChange = true;
		m_OutputUnavailable = false;
		m_LastAudioRetryAttempt = 0;
	}
...
void CProaledClientVoice::PushPeerFrame(int PeerId, const int16_t *pPcm, int Samples, float LeftGain, float RightGain)
{
...
void CProaledClientVoice::MixAudio(int16_t *pOut, int Samples, int OutputChannels)
{
...
void CProaledClientVoice::ClearPeerFrames()
{
	if(m_OutputDevice)
...
void CProaledClientVoice::ResetPeer(SVoicePeer &Peer)
{
	if(m_OutputDevice)
		SDL_LockAudioDevice(m_OutputDevice);
	Peer.m_FrameHead = 0;
...
const char *CProaledClientVoice::FindDeviceName(bool Capture, const char *pDesired) const
{
...
void CProaledClientVoice::ListDevices()
{
	if(!m_pConsole)
		return;
...
void CProaledClientVoice::UpdateMicLevel(float Peak)
{
...
void CProaledClientVoice::Shutdown()
{
	if(m_ShutdownDone)
		return;
	m_ShutdownDone = true;
...
void CProaledClientVoice::UpdateServerAddrConfig()
{
...
void CProaledClientVoice::ResolveServerAddr()
{
...
bool CProaledClientVoice::UpdateContext()
{
...
void CProaledClientVoice::UpdateClientSnapshot()
{
...
void CProaledClientVoice::ProcessCapture()
{
	...
	const int TestMode = std::clamp(Config.m_PcVoiceTestMode, 0, 2);
	const bool TestLocal = TestMode == 1;
	const bool ShowMicLevel = TestMode != 0;
	const bool MicMuted = Config.m_PcVoiceMicMute != 0;
	const float TestGain = std::clamp(Config.m_PcVoiceVolume / 100.0f, 0.0f, 4.0f);
	...
	const int64_t Now = time_get();
	const bool UseVad = Config.m_PcVoiceVadEnable != 0;
	if(!UseVad)
	{
		m_VadActive = false;
	...
	const bool TokenChanged = Config.m_PcVoiceTokenHash != m_LastTokenHashSent;
	...
	if(VoiceAuthValid && (TokenChanged || VoiceAuthChanged || NeedKeepalive))
	{
		...
		WriteU32(aPacket + Offset, Config.m_PcVoiceTokenHash);
		Offset += sizeof(uint32_t);
		...
		m_LastKeepalive = Now;
		m_LastTokenHashSent = Config.m_PcVoiceTokenHash;
	...
	const float VadThreshold = std::clamp(Config.m_PcVoiceVadThreshold / 100.0f, 0.0f, 1.0f);
	const int VadReleaseMs = std::clamp(Config.m_PcVoiceVadReleaseDelayMs, 0, 1000);
...
void CProaledClientVoice::ProcessIncoming()
{
	...
	while(net_socket_read_wait(m_Socket, std::chrono::nanoseconds(0)) > 0)
	{
		...
		if(Type == VOICE_TYPE_PING || Type == VOICE_TYPE_PONG)
		{
			...
			(void)VoiceAuthHash;
			(void)ServerPort;
			if(TokenHash != 0 && TokenHash != Config.m_PcVoiceTokenHash)
				continue;
		...
		if(Type != VOICE_TYPE_AUDIO)
			continue;
		const uint32_t LocalToken = Config.m_PcVoiceTokenHash;
		...
		if(SpecActive && Config.m_PcVoiceHearOnSpecPos)
			LocalPos = SpecPos;
		...
		const bool IgnoreDistance = Config.m_PcVoiceIgnoreDistance || (Config.m_PcVoiceGroupGlobal && SameGroup);
		const char *pSenderName = aSenderName;
		if(!IsSelf)
		{
			const bool AllowObserver = Config.m_PcVoiceHearPeoplesInSpectate && !SenderActive && !SenderSpec;
			if(Config.m_PcVoiceVisibilityMode == 0)
			{
				if(!IgnoreDistance && !SenderActive && !AllowObserver)
					continue;
			}
			else if(Config.m_PcVoiceVisibilityMode == 1)
			{
				if(SenderOtherTeam && !AllowObserver)
					continue;
			}

			if(VoiceListMatch(Config.m_aPcVoiceMute, pSenderName))
				continue;
			if(Config.m_PcVoiceListMode == 1 && !VoiceListMatch(Config.m_aPcVoiceWhitelist, pSenderName))
				continue;
			if(Config.m_PcVoiceListMode == 2 && VoiceListMatch(Config.m_aPcVoiceBlacklist, pSenderName))
				continue;
			const bool SenderUsesVad = (Flags & VOICE_FLAG_VAD) != 0;
			if(SenderUsesVad && !Config.m_PcVoiceHearVad && !VoiceListMatch(Config.m_aPcVoiceVadAllow, pSenderName))
				continue;
		}
		...
		const float Radius = std::max(1, Config.m_PcVoiceRadius) * 32.0f;
		...
		float Volume = std::clamp(RadiusFactor * (Config.m_PcVoiceVolume / 100.0f), 0.0f, 4.0f);
		if(Volume <= 0.0f)
			continue;

		int NameVolume = 100;
		if(VoiceNameVolume(Config.m_aPcVoiceNameVolumes, pSenderName, NameVolume))
		{
		...
		const bool StereoEnabled = Config.m_PcVoiceStereo != 0;
		const float StereoWidth = std::clamp(Config.m_PcVoiceStereoWidth / 100.0f, 0.0f, 2.0f);
...
void CProaledClientVoice::UpdateConfigSnapshot()
{
	std::lock_guard<std::mutex> Guard(m_ConfigMutex);
	m_ConfigSnapshot.m_PcVoiceFilterEnable = g_Config.m_PcVoiceFilterEnable;
	m_ConfigSnapshot.m_PcVoiceNoiseSuppressEnable = g_Config.m_PcVoiceNoiseSuppressEnable;
	m_ConfigSnapshot.m_PcVoiceNoiseSuppressStrength = g_Config.m_PcVoiceNoiseSuppressStrength;
	m_ConfigSnapshot.m_PcVoiceCompThreshold = g_Config.m_PcVoiceCompThreshold;
	m_ConfigSnapshot.m_PcVoiceCompRatio = g_Config.m_PcVoiceCompRatio;
	m_ConfigSnapshot.m_PcVoiceCompAttackMs = g_Config.m_PcVoiceCompAttackMs;
	m_ConfigSnapshot.m_PcVoiceCompReleaseMs = g_Config.m_PcVoiceCompReleaseMs;
	m_ConfigSnapshot.m_PcVoiceCompMakeup = g_Config.m_PcVoiceCompMakeup;
	m_ConfigSnapshot.m_PcVoiceLimiter = g_Config.m_PcVoiceLimiter;
	m_ConfigSnapshot.m_PcVoiceStereo = g_Config.m_PcVoiceStereo;
	m_ConfigSnapshot.m_PcVoiceStereoWidth = g_Config.m_PcVoiceStereoWidth;
	m_ConfigSnapshot.m_PcVoiceRadius = g_Config.m_PcVoiceRadius;
	m_ConfigSnapshot.m_PcVoiceVolume = g_Config.m_PcVoiceVolume;
	m_ConfigSnapshot.m_PcVoiceMicVolume = g_Config.m_PcVoiceMicVolume;
	m_ConfigSnapshot.m_PcVoiceMicMute = g_Config.m_PcVoiceMicMute;
	m_ConfigSnapshot.m_PcVoiceTestMode = g_Config.m_PcVoiceTestMode;
	m_ConfigSnapshot.m_PcVoiceVadEnable = g_Config.m_PcVoiceVadEnable;
	m_ConfigSnapshot.m_PcVoiceVadThreshold = g_Config.m_PcVoiceVadThreshold;
	m_ConfigSnapshot.m_PcVoiceVadReleaseDelayMs = g_Config.m_PcVoiceVadReleaseDelayMs;
	m_ConfigSnapshot.m_PcVoiceIgnoreDistance = g_Config.m_PcVoiceIgnoreDistance;
	m_ConfigSnapshot.m_PcVoiceGroupGlobal = g_Config.m_PcVoiceGroupGlobal;
	m_ConfigSnapshot.m_PcVoiceVisibilityMode = g_Config.m_PcVoiceVisibilityMode;
	m_ConfigSnapshot.m_PcVoiceListMode = g_Config.m_PcVoiceListMode;
	m_ConfigSnapshot.m_PcVoiceDebug = g_Config.m_PcVoiceDebug;
	m_ConfigSnapshot.m_PcVoiceGroupMode = g_Config.m_PcVoiceGroupMode;
	m_ConfigSnapshot.m_PcVoiceHearOnSpecPos = g_Config.m_PcVoiceHearOnSpecPos;
	m_ConfigSnapshot.m_PcVoiceHearPeoplesInSpectate = g_Config.m_PcVoiceHearPeoplesInSpectate;
	m_ConfigSnapshot.m_PcVoiceHearVad = g_Config.m_PcVoiceHearVad;
	m_ConfigSnapshot.m_ClShowOthers = g_Config.m_ClShowOthers;
	uint32_t GroupHash = g_Config.m_PcVoiceToken[0] != '\0' ? str_quickhash(g_Config.m_PcVoiceToken) : 0;
	uint32_t Mode = (uint32_t)std::clamp(g_Config.m_PcVoiceGroupMode, 0, 3);
	m_ConfigSnapshot.m_PcVoiceTokenHash = VoicePackToken(GroupHash, Mode);
	str_copy(m_ConfigSnapshot.m_aPcVoiceWhitelist, g_Config.m_PcVoiceWhitelist, sizeof(m_ConfigSnapshot.m_aPcVoiceWhitelist));
	str_copy(m_ConfigSnapshot.m_aPcVoiceBlacklist, g_Config.m_PcVoiceBlacklist, sizeof(m_ConfigSnapshot.m_aPcVoiceBlacklist));
	str_copy(m_ConfigSnapshot.m_aPcVoiceMute, g_Config.m_PcVoiceMute, sizeof(m_ConfigSnapshot.m_aPcVoiceMute));
	str_copy(m_ConfigSnapshot.m_aPcVoiceVadAllow, g_Config.m_PcVoiceVadAllow, sizeof(m_ConfigSnapshot.m_aPcVoiceVadAllow));
	str_copy(m_ConfigSnapshot.m_aPcVoiceNameVolumes, g_Config.m_PcVoiceNameVolumes, sizeof(m_ConfigSnapshot.m_aPcVoiceNameVolumes));
}

void CProaledClientVoice::GetConfigSnapshot(SRClientVoiceConfigSnapshot &Out) const
{
	std::lock_guard<std::mutex> Guard(m_ConfigMutex);
	Out = m_ConfigSnapshot;
}

void CProaledClientVoice::UpdateEncoderParams()
{
	if(!m_pEncoder)
		return;
...
void CProaledClientVoice::DecodeJitter()
{
	if(!m_OutputDevice)
		return;
...
bool CProaledClientVoice::IsVoiceActive(int ClientId) const
{
...
void CProaledClientVoice::StartWorker()
{
	if(m_Worker.joinable())
		return;
	m_WorkerStop.store(false);
	m_WorkerEnabled.store(true);
	m_Worker = std::thread(&CProaledClientVoice::WorkerLoop, this);
}

void CProaledClientVoice::StopWorker()
{
...
void CProaledClientVoice::WorkerLoop()
{
...
void CProaledClientVoice::OnRender()
{
	if(!g_Config.m_PcVoiceEnable || !m_pGameClient || !m_pClient)
	{
		Shutdown();
		return;
	}
	...
	if(g_Config.m_PcVoiceOffNonActive && m_pGraphics && !m_pGraphics->WindowActive())
	{
		StopWorker();
		if(m_CaptureDevice)
		{
	...
	if(str_comp(m_aAudioBackend, g_Config.m_PcVoiceAudioBackend) != 0)
		NeedReinit = true;
	if(str_comp(m_aInputDeviceName, g_Config.m_PcVoiceInputDevice) != 0)
		NeedReinit = true;
	if(str_comp(m_aOutputDeviceName, g_Config.m_PcVoiceOutputDevice) != 0)
