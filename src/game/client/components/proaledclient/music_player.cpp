...
#if !defined(PC_MUSICPLAYER_HAS_DBUS)
#define PC_MUSICPLAYER_HAS_DBUS 0
#endif

#if defined(CONF_PLATFORM_LINUX) && PC_MUSICPLAYER_HAS_DBUS
#include <dbus/dbus.h>
#endif

#if defined(CONF_FAMILY_WINDOWS) && __has_include(<winrt/Windows.Media.Control.h>)
#define PC_MUSICPLAYER_HAS_WINRT 1
...
#else
#define PC_MUSICPLAYER_HAS_WINRT 0
#endif
...
namespace
{
	...
	static std::string MusicVisualizerBinsSummary(const ProaledClientVisualizer::SVisualizerFrame &Visualizer)
	{
		char aBuf[128];
	...
	struct SNowPlayingSnapshot
	{
		...
		SMusicArt m_Art;
		ProaledClientVisualizer::SVisualizerFrame m_Visualizer;
	};

#if (defined(CONF_PLATFORM_LINUX) && PC_MUSICPLAYER_HAS_DBUS) || PC_MUSICPLAYER_HAS_WINRT
	...
	{
		const int Priority = ProaledClientVisualizer::PlayerSourcePriority(ServiceId);
		if(Priority <= (int)ProaledClientVisualizer::EMediaSourcePriority::DISCORD)
			return -100000;
			...
			if(dbus_message_iter_init(pReply, &RootIter) && dbus_message_iter_get_arg_type(&RootIter) == DBUS_TYPE_ARRAY)
			{
				...
				while(dbus_message_iter_get_arg_type(&ArrayIter) == DBUS_TYPE_STRING)
				{
					...
					if(pName != nullptr && str_startswith(pName, "org.mpris.MediaPlayer2.") &&
						!ProaledClientVisualizer::LooksLikeDiscordPlayer(pName))
...
#if defined(CONF_FAMILY_WINDOWS) && PC_MUSICPLAYER_HAS_WINRT
...
namespace
{
	...
	{
		return g_Config.m_PcMusicPlayerSizeMode == 1;
	}
	...
	{
		return std::clamp(g_Config.m_PcMusicPlayerTextScale / 100.0f, 0.7f, 1.5f);
	}

	static float MusicPlayerHudAlphaScale()
	{
		return std::clamp(g_Config.m_PcMusicPlayerHudColorAlpha / 100.0f, 0.0f, 1.0f);
	}
	...
	{
		return std::clamp(g_Config.m_PcMusicPlayerVisualizerColumns, 5, 10);
	}
	...
	{
		if(g_Config.m_PcMusicPlayerColorMode == 0)
			return BuildPaletteFromAccent(color_cast<ColorRGBA>(ColorHSLA(g_Config.m_PcMusicPlayerStaticColor)));
	...
	{
		if(g_Config.m_PcMusicPlayerColorMode == 0)
			return color_cast<ColorRGBA>(ColorHSLA(g_Config.m_PcMusicPlayerStaticColor));
		return DefaultPreviewAccentForColorMode(g_Config.m_PcMusicPlayerColorMode);
	}
	...
	static ColorRGBA MusicPlayerPanelColor(unsigned BackgroundColor, bool BackgroundEnabled, const SMusicPlayerPalette &Palette, float HoverT)
	{
		...
		const bool TranslucentColorMode = g_Config.m_PcMusicPlayerColorMode == 3;
		const bool CoverColorMode = g_Config.m_PcMusicPlayerColorMode == 1 || g_Config.m_PcMusicPlayerColorMode == 2;
		const bool DominantColorMode = g_Config.m_PcMusicPlayerColorMode == 2;
	...
	static ColorRGBA SelectMusicPlayerAccent(const SArtworkColorAnalysis &Analysis)
	{
		...
		if(!Analysis.m_Valid)
			return DefaultPreviewAccentForColorMode(g_Config.m_PcMusicPlayerColorMode);
		if(g_Config.m_PcMusicPlayerColorMode == 2)
	...
	static SArtCropProfile MusicArtCropProfile(std::string_view ServiceId)
	{
		...
		if(ProaledClientVisualizer::MediaSourceContainsI(ServiceId, "spotify"))
		{
...
public:
	...
	std::unique_ptr<ProaledClientVisualizer::CRealtimeMusicVisualizer> m_pVisualizer;
	SNowPlayingSnapshot m_Snapshot;
	ProaledClientVisualizer::SVisualizerFrame m_VisualizerData;
	...
	ProaledClientVisualizer::EVisualizerBackendStatus m_DebugLastVisualizerBackendStatus = ProaledClientVisualizer::EVisualizerBackendStatus::FALLBACK;
	...
	CImpl()
	{
		...
		m_VisualizerData.m_BackendStatus = ProaledClientVisualizer::EVisualizerBackendStatus::FALLBACK;
#if defined(CONF_PLATFORM_LINUX)
#if PC_MUSICPLAYER_HAS_DBUS
		m_pProvider = std::make_unique<CLinuxNowPlayingProvider>();
#else
		m_pProvider = std::make_unique<CNullNowPlayingProvider>();
#endif
		m_pVisualizer = std::make_unique<ProaledClientVisualizer::CRealtimeMusicVisualizer>();
#elif defined(CONF_FAMILY_WINDOWS) && PC_MUSICPLAYER_HAS_WINRT
	...
	void DebugLogVisualizerState(const char *pSource)
	{
		...
		if(StateChanged)
		{
			MusicPlayerDebugLog(1, "visualizer", "%s: backend=%s signal=%d passive=%d peak=%.4f rms=%.4f bins=%s",
				pSource,
				ProaledClientVisualizer::VisualizerBackendStatusName(m_VisualizerData.m_BackendStatus),
	...
	void DebugLogRenderDecision(const char *pPath, const SNowPlayingSnapshot &Snapshot)
	{
		...
		if(Changed)
		{
			MusicPlayerDebugLog(1, "render", "path=%s playback=%s has_music=%d backend=%s signal=%d",
				...
				ProaledClientVisualizer::VisualizerBackendStatusName(Snapshot.m_Visualizer.m_BackendStatus),
	...
	void Reset(IGraphics *pGraphics)
	{
		...
		m_VisualizerData = ProaledClientVisualizer::SVisualizerFrame();
		m_LastSnapshotTick = 0;
		m_LastPollTick = 0;
	...
	void AttachVisualizerData(SNowPlayingSnapshot &Snapshot) const
	{
		...
		Snapshot.m_Visualizer = ProaledClientVisualizer::SVisualizerFrame();
		if(!m_pVisualizer)
			return;

		Snapshot.m_HasVisualizer = true;
		Snapshot.m_Visualizer = m_VisualizerData;
		if(Snapshot.m_Visualizer.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::LIVE)
	...
	void RefreshVisualizerData()
	{
		...
		ProaledClientVisualizer::SVisualizerPlaybackHint Hint;
		...
		if(m_VisualizerData.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::LIVE)
			m_VisualizerData.m_IsPassiveFallback = false;
		else if(m_VisualizerData.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::FALLBACK || m_VisualizerData.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::UNAVAILABLE)
			m_VisualizerData.m_IsPassiveFallback = true;
	...
	void UpdateVisualizerLevels(CMusicPlayer *pOwner, const SNowPlayingSnapshot &Snapshot, int64_t PositionMs, int RequestedBars, float Delta)
	{
		...
		const float Smoothing = std::clamp(g_Config.m_PcMusicPlayerVisualizerSmoothing / 100.0f, 0.0f, 1.0f);
		...
		const bool UseRealtimeVisualizer =
			...
			Snapshot.m_Visualizer.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::LIVE;
		if(UseRealtimeVisualizer)
		{
			DebugLogRenderDecision("live", Snapshot);
			std::array<float, VISUALIZER_BARS> aTarget{};
			ProaledClientVisualizer::BuildRenderBars(Snapshot.m_Visualizer, aTarget.data(), RequestedBars);
		...
		const bool BackendConnecting = Snapshot.m_HasVisualizer && Snapshot.m_Visualizer.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::CONNECTING;
		const bool BackendSilent = Snapshot.m_HasVisualizer && Snapshot.m_Visualizer.m_BackendStatus == ProaledClientVisualizer::EVisualizerBackendStatus::SILENT;
...
bool CMusicPlayer::GetHudThemeColor(ColorRGBA &Out, bool ForcePreview) const
{
	...
	if(g_Config.m_PcMusicPlayerUseColorForHud == 0)
		return false;
...
void CMusicPlayer::OnUpdate()
{
	...
	if(m_pImpl->m_Snapshot.m_Valid &&
		...
		g_Config.m_PcMusicPlayerShowWhenPaused == 0)
	{
		m_pImpl->ResetHudState();
		return;
	}
...
void CMusicPlayer::RenderMusicPlayer(bool ForcePreview)
{
	if(!m_pImpl)
		return;
	if(!ForcePreview && g_Config.m_PcMusicPlayer == 0)
		return;
	...
	const SMusicPlayerPalette Palette = ForcePreview ?
						    BuildPaletteFromAccent(g_Config.m_PcMusicPlayerColorMode == 0 ? color_cast<ColorRGBA>(ColorHSLA(g_Config.m_PcMusicPlayerStaticColor)) : DefaultPreviewAccentForColorMode(g_Config.m_PcMusicPlayerColorMode)) :
						    m_pImpl->m_Palette;
	const bool TranslucentColorMode = g_Config.m_PcMusicPlayerColorMode == 3;
	const bool CoverColorMode = g_Config.m_PcMusicPlayerColorMode == 1 || g_Config.m_PcMusicPlayerColorMode == 2;
	const bool DominantColorMode = g_Config.m_PcMusicPlayerColorMode == 2;
	...
	if(RenderVisualizer && (!RenderMiniLayout || !MiniControlsVisible))
	{
		const float PositionMs = ForcePreview ? (float)Snapshot.m_PositionMs : m_pImpl->VisualPositionMs(Delta);
			...
			const float VisualizerRoundingT = std::clamp(g_Config.m_PcMusicPlayerVisualizerRounding / 400.0f, 0.0f, 1.0f);
		...
		const float SnappedLaneW = maximum(PixelWidth, roundf(RawLaneW / PixelWidth) * PixelWidth);
			const int VisualizerMode = std::clamp(g_Config.m_PcMusicPlayerVisualizerMode, 0, 2);
