...
namespace
{
	...
	float EffectiveFastInputOffsetTicks(const CGameClient *pGameClient)
	{
		...
		if(PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 1)
		{
	...
	bool EffectiveFastInputOthers()
	{
		...
		if(FastInputMode == 1)
			return g_Config.m_PcDeltaInputOthers != 0;
		if(FastInputMode == 4)
			return g_Config.m_PcSaikoPlusOthers != 0;
		return g_Config.m_PcBestInputOthers != 0;
	}
