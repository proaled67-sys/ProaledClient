...
void CChatBubbles::RenderCurInput(float y)
{
	...
	if(TextContainerIndex.Valid())
	{
		...
		Graphics()->DrawRect(Position.x - FontSize / 2.0f, targetY - FontSize / 2.0f,
			...
			ColorRGBA(0.0f, 0.0f, 0.0f, 0.15f), IGraphics::CORNER_ALL, g_Config.m_PcChatBubbleSize / 4.5f);
...
void CChatBubbles::RenderChatBubbles(int ClientId)
{
	...
	if(!g_Config.m_PcChatBubblesSelf && (ClientId == GameClient()->m_Snap.m_LocalClientId))
		return;

	if(Client()->State() == IClient::STATE_DEMOPLAYBACK && !g_Config.m_PcChatBubblesDemo)
		return;

	const float ShowTime = g_Config.m_PcChatBubbleShowTime / 100.0f;
	int FontSize = g_Config.m_PcChatBubbleSize;
	...
	for(CBubbles &aBubble : m_ChatBubbles[ClientId])
	{
		...
		if(aBubble.m_TextContainerIndex.Valid())
		{
			...
			Graphics()->DrawRect((x - FontSize / 2.0f) + PushBubble, y - FontSize / 2.0f,
				BoundingBox.m_W + FontSize * 1.20f, BoundingBox.m_H + FontSize,
				BgColor, IGraphics::CORNER_ALL, g_Config.m_PcChatBubbleSize / 4.5f);
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		for(auto &aBubble : m_ChatBubbles[i])
		{
			if(aBubble.m_TextContainerIndex.Valid())
			{
				...
				float Posx = Position.x - (BoundingBox.m_W / 2.0f + g_Config.m_PcChatBubbleSize / 15.0f);
...
{
	const float FadeOutTime = g_Config.m_PcChatBubbleFadeOut / 100.0f;
	const float FadeInTime = g_Config.m_PcChatBubbleFadeIn / 100.0f;
	const float ShowTime = g_Config.m_PcChatBubbleShowTime / 100.0f;
...
{
	if(m_UseChatBubbles != g_Config.m_PcChatBubbles)
	{
		m_UseChatBubbles = g_Config.m_PcChatBubbles;
		Reset();
	}
	...
	float ScreenX0, ScreenY0, ScreenX1, ScreenY1;
	Graphics()->GetScreen(&ScreenX0, &ScreenY0, &ScreenX1, &ScreenY1);
	Graphics()->MapScreenToGroup(GameClient()->m_Camera.m_Center.x, GameClient()->m_Camera.m_Center.y, Layers()->GameGroup(), GameClient()->m_Camera.m_Zoom);
