...
namespace
{
	...
	{
		return g_Config.m_PcMusicPlayer != 0 &&
		       !pGameClient->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_MUSIC_PLAYER);
	}
...
CUIRect CHudEditor::GetFallbackModuleRect(HudLayout::EModule Module) const
{
	...
	case HudLayout::MODULE_MOVEMENT_INFO:
	{
		...
		if(g_Config.m_PcShowhudDummyCoordIndicator)
...
CUi::EPopupMenuFunctionResult CHudEditor::PopupModuleSettings(void *pContext, CUIRect View, bool Active)
{
	...
	if(pThis->m_SelectedModule == HudLayout::MODULE_MUSIC_PLAYER)
	{
		...
		if(pThis->GameClient()->m_Menus.DoButton_CheckBox(&g_Config.m_PcMusicPlayerUseColorForHud, BCLocalize("Use Music Player color for HUD"), g_Config.m_PcMusicPlayerUseColorForHud, &MusicPlayerHudColorButton))
			g_Config.m_PcMusicPlayerUseColorForHud ^= 1;

		View.HSplitTop(4.0f, nullptr, &View);
		char aHudColorAlpha[64];
		str_format(aHudColorAlpha, sizeof(aHudColorAlpha), "%s %d%%", BCLocalize("Music Player / HUD alpha"), g_Config.m_PcMusicPlayerHudColorAlpha);
		...
		const float HudAlphaRelative = CUi::ms_LinearScrollbarScale.ToRelative(g_Config.m_PcMusicPlayerHudColorAlpha, 0, 100);
		const float NewHudAlphaRelative = pThis->Ui()->DoScrollbarH(&g_Config.m_PcMusicPlayerHudColorAlpha, &MusicPlayerHudAlphaSlider, HudAlphaRelative);
		g_Config.m_PcMusicPlayerHudColorAlpha = CUi::ms_LinearScrollbarScale.ToAbsolute(NewHudAlphaRelative, 0, 100);
	}
