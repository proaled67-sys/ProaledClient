...
void CTranslate::OnConsoleInit()
{
	...
	if(IsAutoLanguage(g_Config.m_PcTranslateOutgoingTarget))
		str_copy(g_Config.m_PcTranslateOutgoingTarget, DefaultConfig::PcTranslateOutgoingTarget, sizeof(g_Config.m_PcTranslateOutgoingTarget));
...
{
	return g_Config.m_PcTranslateIncomingSource;
}
...
{
	return g_Config.m_PcTranslateOutgoingSource;
}

const char *CTranslate::OutgoingTargetLanguage() const
{
	if(IsAutoLanguage(g_Config.m_PcTranslateOutgoingTarget))
		return DefaultConfig::PcTranslateOutgoingTarget;
	return g_Config.m_PcTranslateOutgoingTarget;
}

bool CTranslate::IsIgnoredIncomingLanguage(const char *pLanguage) const
{
	if(!pLanguage || pLanguage[0] == '\0' || g_Config.m_PcTranslateIncomingIgnoreLanguages[0] == '\0')
		return false;
	...
	const char *pToken = g_Config.m_PcTranslateIncomingIgnoreLanguages;
	while(*pToken != '\0')
	{
