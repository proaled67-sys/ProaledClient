...
void CBackground::OnRender()
{
	...
	const bool MediaBackgroundDisabled = GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_MEDIA_BACKGROUND);
	m_MediaBackground.SyncFromConfig(MediaBackgroundDisabled ? 0 : g_Config.m_PcGameMediaBackground, g_Config.m_PcMenuMediaBackgroundPath);
	m_MediaBackground.Update();
	if(!MediaBackgroundDisabled && g_Config.m_PcGameMediaBackground)
	{
		...
		RenderContext.m_WorldOffset = (float)g_Config.m_PcGameMediaBackgroundOffset / 100.0f;
