...
int CControls::ResolveSnapTapDirection(int Dummy, bool LeftPressed, bool RightPressed)
{
	if(LeftPressed == RightPressed)
	{
		...
		{
			const int64_t Delay = (time_freq() * (int64_t)g_Config.m_PcSnapTapDelay) / 1000;
...
void CControls::GoresMode()
{
	...
	if(!GameClient()->m_Snap.m_pLocalCharacter)
		return;
	if(!g_Config.m_PcGoresMode || GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_GAMEPLAY_GORES_MODE))
		return;

	CCharacterCore Core = GameClient()->m_PredictedPrevChar;

	if(g_Config.m_PcGoresModeDisableIfWeapons)
	{
...
{
	if(g_Config.m_TcFastInput && g_Config.m_PcFastInputMode == 4 && g_Config.m_PcSaikoPlusAmount > 0)
	{
