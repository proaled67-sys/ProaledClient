...
#include <game/client/components/proaledclient/r_jelly.h>
#include <game/client/components/proaledclient/r_trail.h>
	...
	float Intra)
{
	...
	const bool JellyEnabled = !GameClient()->m_ProaledClient.IsComponentDisabled(CProaledClient::COMPONENT_VISUALS_JELLY_TEE);
		...
		ClientId == GameClient()->m_aLocalIds[g_Config.m_ClDummy])
	{
		Graphics()->TextureClear();
		Graphics()->QuadsBegin();
		Graphics()->SetColor(color_cast<ColorRGBA>(ColorHSLA(g_Config.m_PcShowRealHitboxColor, true)));
	...
	if((Player.m_PlayerFlags & PLAYERFLAG_CHATTING) && !GameClient()->m_aClients[ClientId].m_Afk)
	{
		...
		if(g_Config.m_PcEmoticonShadow)
		{
