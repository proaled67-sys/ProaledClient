...
static void RenderProaledClientIcon(IGraphics *pGraphics, const CUIRect &Rect, ColorRGBA Color = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), bool Developer = false)
{
...
static void RenderCenteredProaledClientTabIcon(IGraphics *pGraphics, const CUIRect &Rect, ColorRGBA Color = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f))
{
	CUIRect Icon = Rect;
	...
	RenderProaledClientIcon(pGraphics, Icon, Color);
}
...
{
	if(!g_Config.m_PcUseShortKogServerName)
...
void CMenus::RenderServerbrowserServerList(CUIRect View, bool &WasListboxItemActivated)
{
	...
	enum
	{
		...
		UI_ELEM_MAP_3,
			UI_ELEM_FINISH_ICON,
			UI_ELEM_PROALEDCLIENT_DEV_ICON,
			UI_ELEM_PLAYERS,
			UI_ELEM_FRIEND_ICON,
			UI_ELEM_PROALEDCLIENT_ICON,
			UI_ELEM_PING,
			UI_ELEM_KEY_ICON,
			NUM_UI_ELEMS,
		};
	...
	static SColumn s_aCols[] = {
		...
		{COL_COMMUNITY, -1, "", -1, 28.0f, {0}},
			...
			{COL_PROALEDCLIENT_DEV, -1, "", 1, 20.0f, {0}},
			{COL_PROALEDCLIENT, IServerBrowser::SORT_NUMPROALEDCLIENT, "", 1, 20.0f, {0}},
	...
	for(const auto &Col : s_aCols)
	{
		...
		if(DoButton_GridHeader(&Col.m_Id, Localize(Col.m_pCaption), Checked, &Col.m_Rect))
		{
			...
			else if(Col.m_Id == COL_PROALEDCLIENT)
			{
				const CUIRect Icon = CenterSquareIcon(Col.m_Rect, 2.0f);
				RenderProaledClientIcon(Graphics(), Icon, ColorRGBA(1.0f, 1.0f, 1.0f, 0.9f));
			}
		}
	...
	for(int i = 0; i < NumServers; i++)
	{
		...
		for(const auto &Col : s_aCols)
		{
			...
			else if(Id == COL_MAP)
			{
				...
				else if(Id == COL_PROALEDCLIENT_DEV)
				{
					const bool HasRegularProaledClientPlayers = pItem->m_NumProaledClientPlayers > pItem->m_NumProaledClientDeveloperPlayers;
					if(pItem->m_HasProaledClientDeveloperPlayers && HasRegularProaledClientPlayers)
					{
						const CUIRect Icon = CenterSquareIcon(Button, 2.0f);
						RenderProaledClientIcon(Graphics(), Icon, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), true);

						if(pItem->m_NumProaledClientDeveloperPlayers > 1)
						{
							str_format(aTemp, sizeof(aTemp), "%d", pItem->m_NumProaledClientDeveloperPlayers);
				...
				else if(Id == COL_PROALEDCLIENT)
				{
					if(pItem->m_HasProaledClientPlayers)
					{
						const bool HasDeveloperPlayers = pItem->m_HasProaledClientDeveloperPlayers;
						const int NumRegularProaledClientPlayers = maximum(0, pItem->m_NumProaledClientPlayers - pItem->m_NumProaledClientDeveloperPlayers);
						const bool OnlyDevelopers = HasDeveloperPlayers && NumRegularProaledClientPlayers == 0;
						const CUIRect Icon = CenterSquareIcon(Button, 2.0f);
						RenderProaledClientIcon(Graphics(), Icon, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), OnlyDevelopers);

						const int CounterValue = OnlyDevelopers ? pItem->m_NumProaledClientDeveloperPlayers : NumRegularProaledClientPlayers;
						if(CounterValue > 1)
						{
...
void CMenus::RenderServerbrowserStatusBox(CUIRect StatusBox, bool WasListboxItemActivated)
{
	...
	// address info
	{
		...
		if(GameClient()->m_ProaledClient.HasStreamerFlag(CProaledClient::STREAMER_HIDE_SERVER_IP))
		{
			char aMaskedAddress[128];
			GameClient()->m_ProaledClient.MaskServerAddress(g_Config.m_UiServerAddress, aMaskedAddress, sizeof(aMaskedAddress));
...
void CMenus::ToggleProaledClientServerFilter()
{
...
void CMenus::RenderServerbrowserFilters(CUIRect View)
{
	...
	View.HSplitTop(RowHeight, &Button, &View);
	if(DoButton_CheckBox(&g_Config.m_BrFilterBestclient, Localize("Show ProaledClient only"), g_Config.m_BrFilterBestclient, &Button))
		ToggleProaledClientServerFilter();
...
void CMenus::RenderServerbrowserInfoScoreboard(CUIRect View, const CServerInfo *pSelectedServer)
{
	...
	for(int i = 0; i < pSelectedServer->m_NumReceivedClients; i++)
	{
		...
		else if(CurrentClient.m_aaSkin7[protocol7::SKINPART_BODY][0] != '\0')
		{
			...
			const float ProaledClientIconSize = FontSize - 1.0f;
			const float ProaledClientIconSpacing = 2.0f;
			if(CurrentClient.m_ProaledClient)
				NameText.VSplitRight(ProaledClientIconSize + ProaledClientIconSpacing, &NameText, nullptr);
			...
			if(CurrentClient.m_ProaledClient)
			{
				CUIRect ProaledClientIcon;
				ProaledClientIcon.w = ProaledClientIconSize;
				ProaledClientIcon.h = ProaledClientIconSize;
				ProaledClientIcon.x = minimum(NameCursor.m_X + ProaledClientIconSpacing, Name.x + Name.w - ProaledClientIcon.w);
				ProaledClientIcon.y = Name.y + (Name.h - ProaledClientIcon.h) / 2.0f;
				RenderProaledClientIcon(Graphics(), ProaledClientIcon, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), CurrentClient.m_ProaledClientDeveloper);
			}
...
void CMenus::RenderServerbrowserProaledClient(CUIRect View)
{
	const CServerInfo *pSelectedServer = ServerBrowser()->SortedGet(m_SelectedIndex);
	const float RowHeight = 18.0f;
	...
	View.HSplitTop(RowHeight, &Button, &View);
	if(g_Config.m_PcClientIndicatorVersions)
	{
	...
	int AllProaledClientPlayers = 0;
	for(int i = 0; i < ServerBrowser()->NumSortedServers(); ++i)
		AllProaledClientPlayers += ServerBrowser()->SortedGet(i)->m_NumProaledClientPlayers;

	char aLabel[128];
	str_format(aLabel, sizeof(aLabel), Localize("All ProaledClient players: %d"), AllProaledClientPlayers);
	View.HSplitTop(RowHeight, &Button, &View);
	...
	str_format(aLabel, sizeof(aLabel), Localize("ProaledClient players: %d"), pSelectedServer->m_NumProaledClientPlayers);
	View.HSplitTop(RowHeight, &Button, &View);
	Ui()->DoLabel(&Button, aLabel, FontSize, TEXTALIGN_ML);

	View.HSplitTop(4.0f, nullptr, &View);
	if(!pSelectedServer->m_HasProaledClientPlayers)
	{
		Ui()->DoLabel(&View, Localize("No ProaledClient users on the selected server"), FontSize, TEXTALIGN_MC);
		return;
	}

	std::vector<int> vProaledClientIndexes;
	vProaledClientIndexes.reserve(pSelectedServer->m_NumReceivedClients);
	for(int i = 0; i < pSelectedServer->m_NumReceivedClients; ++i)
	{
		if(pSelectedServer->m_aClients[i].m_ProaledClient)
			vProaledClientIndexes.push_back(i);
	}
	...
	s_ListBox.SetScrollbarWidth(16.0f);
	s_ListBox.SetScrollbarMargin(5.0f);
	s_ListBox.DoStart(26.0f, (int)vProaledClientIndexes.size(), 1, 1, -1, &View, false, IGraphics::CORNER_NONE, true);

	for(size_t i = 0; i < vProaledClientIndexes.size(); ++i)
	{
		const CServerInfo::CClient &Client = pSelectedServer->m_aClients[vProaledClientIndexes[i]];
		...
		CUIRect NameText = Name;
		const float ProaledClientIconSize = FontSize;
		const float ProaledClientIconSpacing = 2.0f;
		NameText.VSplitRight(ProaledClientIconSize + ProaledClientIconSpacing, &NameText, nullptr);
		CTextCursor NameCursor;
		...
		ProaledClientIcon.x = minimum(NameCursor.m_X + ProaledClientIconSpacing, Name.x + Name.w - ProaledClientIcon.w);
		ProaledClientIcon.y = Name.y + (Name.h - ProaledClientIcon.h) / 2.0f;
		RenderProaledClientIcon(Graphics(), ProaledClientIcon, ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), Client.m_ProaledClientDeveloper);
...
void CMenus::RenderServerbrowserTabBar(CUIRect TabBar)
{
	...
	CUIRect FilterTabButton, InfoTabButton, ProaledClientTabButton, FriendsTabButton;
	TabBar.VSplitLeft(TabBar.w / 4.0f, &FilterTabButton, &TabBar);
	TabBar.VSplitLeft(TabBar.w / 3.0f, &InfoTabButton, &TabBar);
	TabBar.VSplitLeft(TabBar.w / 2.0f, &ProaledClientTabButton, &FriendsTabButton);
	...
	static CButtonContainer s_ProaledClientTabButton;
	if(DoButton_MenuTab(&s_ProaledClientTabButton, "", g_Config.m_UiToolboxPage == UI_TOOLBOX_PAGE_PROALEDCLIENT, &ProaledClientTabButton, IGraphics::CORNER_T, &m_aAnimatorsSmallPage[SMALL_TAB_BROWSER_PROALEDCLIENT], &ColorInactive, &ColorActive))
	{
		g_Config.m_UiToolboxPage = UI_TOOLBOX_PAGE_PROALEDCLIENT;
	}
	RenderCenteredProaledClientTabIcon(Graphics(), ProaledClientTabButton, ColorRGBA(1.0f, 1.0f, 1.0f, 0.95f));
	GameClient()->m_Tooltips.DoToolTip(&s_ProaledClientTabButton, &ProaledClientTabButton, Localize("ProaledClient"));
...
void CMenus::RenderServerbrowserToolBox(CUIRect ToolBox)
{
	...
	case UI_TOOLBOX_PAGE_INFO:
		...
		return;
	case UI_TOOLBOX_PAGE_PROALEDCLIENT:
		RenderServerbrowserProaledClient(ToolBox);
		return;
