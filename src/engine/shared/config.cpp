...
#undef MACRO_CONFIG_STR
	...
	m_pConsole->Register("tc_fast_input_mode", "?i", CFGFLAG_CLIENT, Con_TcFastInputModeLegacy, this, "Alias for pc_fast_input_mode");
	m_pConsole->Register("tc_fast_input_delta_input", "?i", CFGFLAG_CLIENT, Con_TcFastInputDeltaInputLegacy, this, "Alias for pc_fast_input_delta_input");
	m_pConsole->Register("tc_delta_input_others", "?i", CFGFLAG_CLIENT, Con_TcDeltaInputOthersLegacy, this, "Alias for pc_delta_input_others");
...
void CConfigManager::Con_TcFastInputModeLegacy(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		char aCmd[64];
		str_format(aCmd, sizeof(aCmd), "pc_fast_input_mode %d", pResult->GetInteger(0));
		pConfigManager->m_pConsole->ExecuteLine(aCmd, pResult->m_ClientId);
	}
	else
	{
		pConfigManager->m_pConsole->ExecuteLine("pc_fast_input_mode", pResult->m_ClientId);
	}
}
...
void CConfigManager::Con_TcFastInputDeltaInputLegacy(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		char aCmd[64];
		str_format(aCmd, sizeof(aCmd), "pc_fast_input_delta_input %d", pResult->GetInteger(0));
		pConfigManager->m_pConsole->ExecuteLine(aCmd, pResult->m_ClientId);
	}
	else
	{
		pConfigManager->m_pConsole->ExecuteLine("pc_fast_input_delta_input", pResult->m_ClientId);
	}
}
...
void CConfigManager::Con_TcDeltaInputOthersLegacy(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		char aCmd[64];
		str_format(aCmd, sizeof(aCmd), "pc_delta_input_others %d", pResult->GetInteger(0));
		pConfigManager->m_pConsole->ExecuteLine(aCmd, pResult->m_ClientId);
	}
	else
	{
		pConfigManager->m_pConsole->ExecuteLine("pc_delta_input_others", pResult->m_ClientId);
	}
}
