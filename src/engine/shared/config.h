...
static constexpr int PC_FAST_INPUT_GAMMA_UI_MAX = 600;

constexpr int PcFastInputGammaUiToEffectiveAmount(int GammaUiAmount)
{
	if(GammaUiAmount <= 0)
		return 0;
	if(GammaUiAmount >= PC_FAST_INPUT_GAMMA_UI_MAX)
		return PC_FAST_INPUT_GAMMA_UI_MAX;
	return GammaUiAmount;
}

constexpr int PcFastInputGammaEffectiveToUiAmount(int EffectiveAmount)
{
	if(EffectiveAmount <= 0)
		return 0;
	if(EffectiveAmount >= PC_FAST_INPUT_GAMMA_UI_MAX)
		return PC_FAST_INPUT_GAMMA_UI_MAX;
	return EffectiveAmount;
}

constexpr int PcFastInputNormalizedMode(int Mode)
{
	if(Mode == 2)
		return 3;
	return Mode;
