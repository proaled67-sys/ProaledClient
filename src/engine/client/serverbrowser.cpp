...
void CServerBrowser::SetProaledClientPlayers(const std::vector<CProaledClientPlayerEntry> &vPlayers)
{
	m_ProaledClientPlayersByServer.clear();
	for(const auto &Entry : vPlayers)
	{
		if(Entry.m_aServerAddress[0] == '\0' || Entry.m_aName[0] == '\0')
			continue;
		bool &Developer = m_ProaledClientPlayersByServer[Entry.m_aServerAddress][Entry.m_aName];
		Developer = Developer || Entry.m_Developer;
	}

	for(CServerEntry *pEntry : m_vpServerlist)
	{
		UpdateServerProaledClients(&pEntry->m_Info);
	}
...
bool CServerBrowser::SortCompareNumProaledClientPlayers(int Index1, int Index2) const
{
	CServerEntry *pIndex1 = m_vpServerlist[Index1];
	CServerEntry *pIndex2 = m_vpServerlist[Index2];

	if(pIndex1->m_Info.m_NumProaledClientPlayers == pIndex2->m_Info.m_NumProaledClientPlayers)
		return pIndex1->m_Info.m_NumFilteredPlayers > pIndex2->m_Info.m_NumFilteredPlayers;
	else
		return pIndex1->m_Info.m_NumProaledClientPlayers > pIndex2->m_Info.m_NumProaledClientPlayers;
}
...
void CServerBrowser::Filter()
{
	...
	for(int ServerIndex = 0; ServerIndex < (int)m_vpServerlist.size(); ServerIndex++)
	{
		...
		if(!Filtered)
		{
			...
			UpdateServerProaledClients(&Info);

			if((!g_Config.m_BrFilterFriends || Info.m_FriendState != IFriends::FRIEND_NO) &&
				(!g_Config.m_BrFilterBestclient || Info.m_HasProaledClientPlayers))
			{
...
void CServerBrowser::Sort()
{
	...
	else if(g_Config.m_BrSort == IServerBrowser::SORT_NUMPROALEDCLIENT)
		std::stable_sort(m_vSortedServerlist.begin(), m_vSortedServerlist.end(), CSortWrap(this, &CServerBrowser::SortCompareNumProaledClientPlayers));
...
void CServerBrowser::UpdateServerProaledClients(CServerInfo *pInfo) const
{
	pInfo->m_NumProaledClientPlayers = 0;
	pInfo->m_HasProaledClientPlayers = false;
	pInfo->m_NumProaledClientDeveloperPlayers = 0;
	pInfo->m_HasProaledClientDeveloperPlayers = false;
	for(auto &Client : pInfo->m_aClients)
	{
		Client.m_ProaledClient = false;
		Client.m_ProaledClientDeveloper = false;
	}
	...
	for(int AddressIndex = 0; AddressIndex < pInfo->m_NumAddresses; ++AddressIndex)
	{
		...
		const auto It = m_ProaledClientPlayersByServer.find(aAddress);
		if(It != m_ProaledClientPlayersByServer.end())
	...
	for(int ClientIndex = 0; ClientIndex < NumClients; ++ClientIndex)
	{
		...
		for(const auto *pPlayers : vpAddressMatches)
		{
			...
			if(PlayerIt == pPlayers->end())
				continue;
			Client.m_ProaledClient = true;
			Client.m_ProaledClientDeveloper = Client.m_ProaledClientDeveloper || PlayerIt->second;
		}
		if(Client.m_ProaledClient)
		{
			pInfo->m_NumProaledClientPlayers++;
			if(Client.m_ProaledClientDeveloper)
				pInfo->m_NumProaledClientDeveloperPlayers++;
		}
	}

	pInfo->m_HasProaledClientPlayers = pInfo->m_NumProaledClientPlayers > 0;
	pInfo->m_HasProaledClientDeveloperPlayers = pInfo->m_NumProaledClientDeveloperPlayers > 0;
}
