...
static const char *PROALEDCLIENT_SERVERLIST_URL = "https://master.proaledclient.fun/servers.json";
...
private:
	...
	bool m_UseProaledClientMaster = false;
...
CServerBrowserHttp::CServerBrowserHttp(IEngine *pEngine, IStorage *pStorage, IHttp *pHttp, const char **ppUrls, int NumUrls, int PreviousBestIndex) :
	...
	m_UseProaledClientMaster(g_Config.m_PcMastersrv != 0)
{
	Refresh();
}
...
{
	const bool UseProaledClientMaster = g_Config.m_PcMastersrv != 0;
	char aaUrls[CChooseMaster::MAX_URLS][256];
	const char *apUrls[CChooseMaster::MAX_URLS] = {nullptr};
	int NumUrls = 0;
	if(UseProaledClientMaster)
	{
		apUrls[0] = PROALEDCLIENT_SERVERLIST_URL;
		NumUrls = 1;
	}
	else
	{
	...
	m_UseProaledClientMaster = UseProaledClientMaster;
...
{
	if(m_UseProaledClientMaster != (g_Config.m_PcMastersrv != 0))
	{
