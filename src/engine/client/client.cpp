...
static void CleanupLegacyReShadeFiles()
{
	...
	if(fs_storage_path("ProaledClient", aUserDir, sizeof(aUserDir)) != 0)
		return;
	...
	static constexpr const char *s_apFilesToRemove[] = {
		...
		"ProaledClientReShadeBridge.ini",
		"proaledclient_reshade_live.addon",
		"proaledclient_reshade_bridge.addon",
	};
...
void CClient::SendBClientInfo(int Conn)
{
	...
	{
		CMsgPacker Msg(NETMSG_IAMPROALEDCLIENT, true);
		Msg.AddString(PROALEDCLIENT_VERSION " built on " __DATE__ ", " __TIME__);
	...
	}
	{
		...
		CMsgPacker Msg(NETMSG_PROALEDCLIENT_EXESIZE, true);
		Msg.AddString(aBuf);
		SendMsg(Conn, &Msg, MSGFLAG_VITAL);
	}
}
		...
		if(m_aapSnapshots[g_Config.m_ClDummy][SNAP_CURRENT])
		{
			...
			if(m_aapSnapshots[g_Config.m_ClDummy][SNAP_PREV])
			{
				...
				const bool HasFastInput =
					...
					((PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 0 && g_Config.m_TcFastInputAmount > 0) ||
						(PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 1 && g_Config.m_PcFastInputDeltaInput > 0) ||
						(PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 3 && g_Config.m_PcBestInputOffset > 0) ||
						(PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 4 && g_Config.m_PcSaikoPlusAmount > 0));
				if(HasFastInput && PcFastInputNormalizedMode(g_Config.m_PcFastInputMode) == 4)
				{
...
namespace
{
	...
	ECrashReportDialogAction ShowCrashReportDialogWindows(const char *pTitle, const char *pDetails, bool CanRestart, bool CanOpenDumps)
	{
		...
		static const wchar_t *s_pClassName = L"ProaledClientCrashReportDialog";
...
#if defined(CONF_FAMILY_WINDOWS)
		const ECrashReportDialogAction Action = ShowCrashReportDialogWindows("ProaledClient Crash Handler", pDetailedMessage, HasStorage, HasStorage);
...
int CClient::PredictionMargin() const
{
	...
	if(!g_Config.m_PcFastInputAutoMargin)
	...
	if(g_Config.m_TcFastInput)
	{
		...
		{
			const int DeltaInputAmount = std::max(0, g_Config.m_PcFastInputDeltaInput);
			FastInputMargin = (DeltaInputAmount + 2) / 5;
		}
		else if(FastInputMode == 4)
		{
			const int SaikoPlusAmount = std::max(0, g_Config.m_PcSaikoPlusAmount);
			FastInputMargin = (SaikoPlusAmount + 2) / 5;
		}
		else
		{
			const int BestInputAmount = std::max(0, g_Config.m_PcBestInputOffset);
